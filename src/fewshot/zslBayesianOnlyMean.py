...
def createModel(data=None,
    ...
    # Few shot learning
    # See some data for each class, say 3 data points
    # Weigh them according to the Kernel
    ...
    for i in unseenList:
        for j in range(3):
            print("For i and j", i, j, "we get distance")
            k = 0
            for l in range(4096):
                k += testD[i][j][l] * testD[i][j][l] * (1 / empSigOut[i][l])
            k = k / 50
            muOut[i] = (muOut[i] + k * testD[i][j]) / (1 + k)
